 ...
 * Scrapes the URL and uses Google Search to find core identity and contact details.
 */
// Exported to fix error: Module '"./services/geminiService"' declares 'investigatorAgent' locally, but it is not exported.
export const investigatorAgent = async (ai: GoogleGenAI, url: string, customInstruction: string) => {
 ...
 * Researches local competitors and typical service demand for the identified region.
 */
// Exported to fix error: Module '"./services/geminiService"' declares 'marketAnalystAgent' locally, but it is not exported.
export const marketAnalystAgent = async (ai: GoogleGenAI, businessData: any) => {
  const response = await ai.models.generateContent({
    model: 'gemini-3-flash-preview',
 ...
 * Develops a logical pricing model and core item list based on industry data.
 */
// Exported to fix error: Module '"./services/geminiService"' declares 'pricingStrategistAgent' locally, but it is not exported.
export const pricingStrategistAgent = async (ai: GoogleGenAI, businessData: any) => {
  const response = await ai.models.generateContent({
...
/**
 * AGENT 4: Content Copywriter & Brand Stylist
 * Curates headers, sub-headers, and upsell packages to maximize conversions.
 */
// Exported to fix error: Module '"./services/geminiService"' declares 'copywriterAgent' locally, but it is not exported.
export const copywriterAgent = async (ai: GoogleGenAI, businessData: any) => {
  const response = await ai.models.generateContent({
    model: 'gemini-3-flash-preview',
...
export const performMasterScan = async (url: string, customInstruction?: string): Promise<Partial<BusinessConfig>> => {
  return retryRequest(async () => {
    ...
    // Step 2: Parallel Market, Pricing, and Creative analysis
...
export const getEstimate = async (task: EstimateTask, config: BusinessConfig): Promise<EstimationResult> => {
  return retryRequest(async () => {
    ...
    const response = await ai.models.generateContent({
      model: 'gemini-3-flash-preview',
      contents: `As an AI estimation agent for ${config.name}, calculate costs for: "${task.description}".
      Zip Code: ${task.zipCode}.
      Pricing Knowledge: ${config.pricingRules}.
      Available Upsells:
